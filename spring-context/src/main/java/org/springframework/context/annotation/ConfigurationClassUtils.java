...
abstract class ConfigurationClassUtils {
	...
	public static boolean checkConfigurationClassCandidate(
		...
		/**
		 * 通过注解注入的BeanDefinition都是AnnotatedGenericBeanDefinition，实现了 {@link AnnotatedBeanDefinition}
		 * Spring内部的BeanDefinition是RootBeanDefinition，实现了 {@link AbstractBeanDefinition}
		 *
		 * 根据不同注解类型的 BeanDefinition，采用不同的方式获取到对应的 {@link AnnotationMetadata}
		 */

		// step 1: 根据不同注解类型的 BeanDefinition，采用不同的方式获取到对应的 {@link AnnotationMetadata}

		// 此处主要用于判断是否归属于注解定义的AnnotatedBeanDefinition
		...
		// 如果bean被@configuration注解标注
		// 且被注解@Component，@ComponentScan、@Import、@ImportResource或者@Bean标记的方法，则将bean定义标记为lite
